// Versão atualizada com correções de tipos e uso correto dos atributos
"use client";
...
export default function ProductCard({
  ...
  const imagem = product.imagens?.[0] || "/fallback.png";
  const precoPix = formatPrecoToNumber(product.valor);
  const precoPromo = product.promocao ? formatPrecoToNumber(product.valor) : 0;
  const precoFinal = precoPromo > 0 ? precoPromo : precoPix;

  const { taxa12x } = useCatalogo();
  const mostrarParcelamento = precoFinal > 50 && taxa12x !== null;
  const totalComJuros = precoFinal * (1 + (taxa12x || 0) / 100);
  const parcela12x = totalComJuros / 12;

  const linkZap = `https://wa.me/${
    loja.whatsapp
  }?text=Tenho%20interesse%20no%20produto%20${encodeURIComponent(
    product.titulo
  )}`;
  ...
  return (
    <motion.div
      ...
      className={`group border border-gray-200 hover:border-gray-400 rounded-2xl overflow-hidden shadow-sm bg-white transition cursor-pointer ${
        isLista
          ? "flex flex-row w-full min-h-[160px]"
      ...
      <Link href={`/produtos/${product.slug}`} className="block h-full w-full">
        ...
        >
          ...
          <Image
            ...
            className={`transition-all duration-300 group-hover:scale-110 ${
              isLista ? "object-cover" : "object-contain"
            }`}
          />
        </div>
        ...
        <div className="flex flex-col justify-between p-4 flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-2 text-[11px] font-medium text-gray-700 mt-2 items-center min-h-[1.5rem]">
            ...
            {getAtributo("ram") && (
              <span className="bg-gray-100 px-2 py-0.5 rounded inline-flex items-center gap-1">
                <MdMemory size={12} /> {getAtributo("ram")}
              </span>
            )}
            {getAtributo("armazenamento") && (
              <span className="bg-gray-100 px-2 py-0.5 rounded inline-flex items-center gap-1">
                <RiSdCardMiniLine size={12} /> {getAtributo("armazenamento")}
              </span>
            )}
            {product.nfc && (
          ...
          <div className="mt-1 min-h-[4.5rem]">
            {product.promocao && (
              <p className="text-xs text-gray-400 line-through">
                {formatPreco(precoPix)}
              </p>
            )}
            <p className="text-lg font-bold text-green-600">
              {formatPreco(precoFinal)} <span className="text-sm">no pix</span>
            </p>
            {mostrarParcelamento && (
